...
export const columns = [
  {
    ...
    cell: (row) => (
      <div class="avatar me-1" width="50" height="50">
        <img src={row?.product?.image || ""} alt="" width="32" height="32" />
      </div>
    ),
  },
  {
    ...
    cell: (row) => {
      return (
        (row?.product?.product_names && row?.product?.product_names[0]?.name) ||
        ""
      );
    },
  },
  {
    ...
    cell: (row) => (
      <span className="text-capitalize">
        {number_to_price((row?.price && row?.price) || 0) +
          " " +
          getMoneyByLang(row?.lang)}
      </span>
    ),
  },
  {
    ...
    cell: (row) => (
      <span className="text-capitalize">
        {number_to_price((row?.discount && row?.discount) || 0) +
          " " +
          getMoneyByLang(row?.lang)}
      </span>
    ),
  },
  {
    ...
    selector: "vat",
    cell: (row) =>
      String(((row?.product?.vat && row?.product?.vat) || 0) + "%"),
  },
  {
  ...
  },
  {
    ...
    cell: (row) =>
      row?.transaction && row?.transaction?.startDate
        ? formatDateTime(row?.transaction?.startDate)
        : "",
  },
  {
    ...
    selector: "transaction",
    cell: (row) =>
      row?.transaction && row?.transaction?.endDate
        ? formatDateTime(row?.transaction?.endDate)
        : "",
  },
  {

  ...
  useEffect(() => {
    (async () => {
      ...
      if (listAbout?.data?.data?.length > 0) {
        setCloud(listAbout?.data?.data[0]?.id);
        setValueDescription(
          htmlToDraftUtil(listAbout?.data?.data[0]?.my_cloud || " ")
        );
      }
    })();
  }, []);
...
export default injectIntl(CloudTab);

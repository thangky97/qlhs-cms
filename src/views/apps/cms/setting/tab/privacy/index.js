    ...
    <div className="invoice-list-table-header w-100 mr-1 ml-50 ">
      {
        <Row className="justify-content-end">
          ...
          >
            <Button.Ripple
              color="primary"
              onClick={() => history.push("/apps/cms/setting/privacy/edit")}
            >
...
const CityList = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (listAbout?.data?.data?.length > 0) {
        setPrivacyData(listAbout?.data?.data[0]);
      }
    })();
  }, []);
  const toggleSidebar = () => {
  ...
  return (
    <div className="app-user-list">
      ...
      <ExtensionsHeader title={<FormattedMessage id="privacy" />} />
     <Row >
      <Col md={6}>
      <Card className="card-apply-job " style={{ height: "unset" }}>
        <CardBody>
          <div  dangerouslySetInnerHTML={{
                  __html: privacyData?.privacy_policy+ "",
                }}>

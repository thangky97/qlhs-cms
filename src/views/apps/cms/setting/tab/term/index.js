    ...
    <div className="invoice-list-table-header w-100 mr-1 ml-50 ">
      {
        <Row className="justify-content-end">
          ...
          >
            <Button.Ripple
              color="primary"
              onClick={() => history.push("/apps/cms/setting/term/edit")}
            >
...
const CityList = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (listAbout?.data?.data?.length > 0) {
        setfaqData(listAbout?.data?.data[0]);
      }
    })();
  }, []);
  const toggleSidebar = () => {
  ...
  return (
    <div className="app-user-list">
      ...
      <ExtensionsHeader title={<FormattedMessage id="terms of use" />} />
     <Row >
      <Col md={6}>
      <Card className="card-apply-job " style={{ height: "unset" }}>
        <CardBody>
          <div  dangerouslySetInnerHTML={{
                  __html: faqData?.terms_of_use+ "",
                }}>
          </div>

...
import { updateTrainingProgram } from "../store/action";
import { useHistory } from "react-router-dom";

const EditTrainingProgram = ({ selected, intl }) => {
  ...
  const cate = yup.object({
    code: yup
      .string()
      .required(<FormattedMessage id="Mã chương trình học là bắt buộc" />),
    name: yup
      .string()
      .required(<FormattedMessage id="Tên chương trình học là bắt buộc" />),
  });
  const {
    register,
    formState: { errors },
  ...
  return (
    <Card className="invoice-preview-card mb-0 p-3">
      <ExtensionsHeader
        title={<FormattedMessage id="Chỉnh sửa chương trình học" />}
      />

      <Form onSubmit={handleSubmit(onSubmit)}>
        <FormGroup>
          <Label for="code">
            <FormattedMessage id="Mã chương trình học" />{" "}
        ...
        <FormGroup>
          <Label for="name">
            <FormattedMessage id="Tên chương trình học" />{" "}
            <span className="text-danger">*</span>
          </Label>
          <Input
            name="name"
...
export default injectIntl(EditTrainingProgram);

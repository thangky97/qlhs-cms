...
import { getDataTrainingProgram } from "../store/action";
...
const TrainingProgramList = ({ intl }) => {
  const dispatch = useDispatch();
  const common = useSelector((state) => state.common);
  const training_programs = useSelector((state) => state.training_programs);
  ...
  }, [loadData, training_programs?.statusCode]);
  ...
  const count = Number(Math.ceil(training_programs?.total / rowsPerPage));
  ...
  const dataToRender = () => {
    if (training_programs?.allData?.length > 0) {
      return training_programs?.allData;
    }
  };
  ...
  useEffect(() => {
    switch (training_programs?.type) {
      case "ADD_TRAINING_PROGRAM":
        if (training_programs?.statusCode == 200) {
          toast.success(<FormattedMessage id={"successfully added new!"} />);
          setLoadData(!loadData);
          // toggleSidebar();
        } else if (training_programs?.statusCode == 400) {
  ...
  return (
    <div className="app-user-list">
      <ExtensionsHeader title={<FormattedMessage id="trainingProgram" />} />
        ...
        <CardBody>
          <Row>
            ...
            <Col md="3">
              <Input
                className=" w-100"
                placeholder={intl.formatMessage({ id: "training_program" })}
                type="text"
                value={Name}
...
export default injectIntl(TrainingProgramList);

...
import { getDataClassroom } from "../store/action";
...
const ClassroomList = ({ intl }) => {
  ...
  }, [loadData, classrooms?.statusCode]);
  ...
  const count = Number(Math.ceil(classrooms?.total / rowsPerPage));
  ...
  const dataToRender = () => {
    if (classrooms?.allData?.length > 0) {
      return classrooms?.allData;
    }
  };
  ...
  useEffect(() => {
    switch (classrooms?.type) {
      case "ADD_CLASSROOM":
        if (classrooms?.status?.statusCode == 200) {
          toast.success(<FormattedMessage id={"successfully added new!"} />);
          setLoadData(!loadData);
          // toggleSidebar();
        } else if (classrooms?.status?.statusCode == 400) {
  ...
  return (
    <div className="app-user-list">
      <ExtensionsHeader title={<FormattedMessage id="Quản lý phòng học" />} />
...
export default injectIntl(ClassroomList);
